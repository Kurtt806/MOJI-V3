...
bool otaHttpUpdate(const char *fwInfoUrl)
{
  ...
  // --- Tải firmware ---
  HTTPClient httpFw;
  // Use a WiFiClientSecure instance so we can control cert verification /
  // optionally skip verification if device runs out of RAM for full TLS.
  WiFiClientSecure *client = new WiFiClientSecure();
  // If you have the server CA use client->setCACert(ca_pem);
  // Otherwise to avoid heavy cert handling and reduce memory usage:
  client->setInsecure(); // NOTE: skips certificate verification

  // follow redirects (fix for HTTP 302 from servers like GitHub/CloudFront)
