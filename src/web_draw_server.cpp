                             ...
                             uint8_t *data, size_t len, bool final)
{
  if (!index)
  {
    if (SPIFFS.exists("/anim.gif"))
      SPIFFS.remove("/anim.gif");
  }

  File uploadFile = SPIFFS.open("/anim.gif", FILE_APPEND);
  if (uploadFile)
  {
  ...
  {
    // If opening the file failed, you could log or handle the error here.
    // We don't send an immediate error response because uploads are chunked.
  }
